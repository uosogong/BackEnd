...
class DepartmentServiceTest {
    ...
    fun `should return department list with ratings and dibs`() {
        ...
        val departments = listOf(
            Department(id = 1L, name = "Dept A", introduction = "aa", internRecruitment = false, scholarshipRecruitment = false, user = mockuser1),
            Department(id = 2L, name = "Dept B", introduction = "aa", internRecruitment = false, scholarshipRecruitment = false, user = mockuser2)
        )
        every { departmentRepository.findAllByOrderByUpdatedDateDesc() } returns departments
        every { feedbackService.findDepartmentFeedbacks(1L) } returns FeedbackResponseDto.FeedbackListDto()
        every { feedbackService.findDepartmentFeedbacks(2L) } returns FeedbackResponseDto.FeedbackListDto()
